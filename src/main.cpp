...
//
//   Encoder            ESP32 (bancada / VSPI padrão)
//   ---------------------------------------------------
//   CLK   (via 4) ───► GPIO18 (SCK)          direto
//   MOSI  (via 6) ───► GPIO23 (MOSI)         direto
//   NCS   (via 3) ───► GPIO5  (NCS)          direto
//   MISO  (via 5) ──[LEVEL SHIFTER 5V→3V3]─► GPIO19 (MISO)
...
//   constexpr int8_t PIN_MISO = 35;  // GPIO34/35/36/39 são só-entrada; MISO continua
//                                    // exigindo level shifter (74HC245 3,3 V não é 5 V-tol.)
